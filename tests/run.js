...
describe("Sessions", function() {
  before(function(done) {
    ...
    support.credentials().then(function(auth) {
      credentails = auth;
      done();
    });
  });
  ...
  it("should not be problem to create sessions", function(done) {
    ...
    var storage = new Client.CookieFileStorage(
      __dirname + "/cookies/" + credentails[0] + ".json"
    );
    var promise = Client.Session.create(
      device,
      storage,
      credentails[0],
      credentails[1],
  ...
  describe("Samples", function() {
    ...
    it("should not be problem to search account with session", function(done) {
      Client.Account.searchForUser(session, "instagram").then(function(
        account
      ) {
    ...
    it("should not be problem to show discover feed", function(done) {
      Client.discover(session, false, 5).then(function(discover) {
    ...
    it("should be able to block user", function(done) {
      Client.Relationship.block(session, "1750777689").then(function(
        relationship
      ) {
    ...
    it("should not be problem to access media location property", function(done) {
      ...
      Client.Location.search(session, "New York")
        .then(function(locations) {
          ...
          var locationFeed = new Client.Feed.LocationMedia(
            session,
    ...
    it("should not be problem to search hashtag", function(done) {
      Client.Hashtag.search(session, "instagram").then(function(hashtags) {
